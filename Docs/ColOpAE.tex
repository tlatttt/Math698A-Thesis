...
connect/.style={->,thick}]
\draw [->, draw=black, very thick] (-4, 4) -- node[below] {$\tau = \psi(f)$} (-1, 4);
\draw [->, draw=black, very thick] (0.5, 4) -- node[below] {$\rho = \Theta(\tau)$} (3.5, 4);
\draw [->, draw=black, very thick]  (5, 4) -- node[below] {$f = \psi^+(r)$} (8, 4);
\node [rotate=90] at (-5, 0) { discrete solution};
\node [rotate=90] at (9, 0) {Collision operation approximation};
